...
-(void)setSubview{
   ...
   // [self.phoneNum.btn addTarget:self action:@selector(getCheak:) forControlEvents:UIControlEventTouchUpInside];
    ...
    self.pass.field.keyboardType = UIKeyboardTypeNumberPad;
    ...
    self.passRepeat.field.keyboardType = UIKeyboardTypeNumberPad;
...
- (TimerButton *)pinBtn {
    if (!_pinBtn) {
        ...
        _pinBtn = [[TimerButton alloc] initWithFrame:frame title:@"验证" durationTitle:@"验证" seconds:60 progressBlock:^(TimerButton *button, TimeState state, NSString *restTime) {
            if (state == TimeStart) {
