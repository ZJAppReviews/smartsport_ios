...
@interface matchDetailVC ()<UITableViewDelegate,UITableViewDataSource ,UICollectionViewDataSource,UICollectionViewDelegate,UICollectionViewDelegateFlowLayout>
...
-(void)setsubview{
    ...
    self.price = [XYUIKit labelWithTextColor:KmainRed numberOfLines:0 text:@"¥999999" fontSize:17];
    ...
    .heightIs(18)
    .centerYEqualToView(self.styleL);
    [self.numS setSingleLineAutoResizeWithMaxWidth:150];
    ...
    [self requestType:HttpRequestTypePost
         ...
         successBlock:^(BaseModel *response) {
            ...
            // NSArray * arr1 = response.data.match_imgs;
             ...
             [self.bottomBtn setTitle: [NSString stringWithFormat:@"我要报名（¥%@)",self.model.price] forState:UIControlStateNormal];
             [self.headImg sd_setImageWithURL:[NSURL URLWithString:self.model.cover] placeholderImage:nil];
             if([self.model.statusV isEqualToString:@"报名中"]){
                 _tag.backgroundColor = mainColor ;
             }else if([self.model.statusV isEqualToString:@"直播中"]){
                 _tag.backgroundColor = KRgb(0.35, 0.78, 0.98) ;
             }else{
                 _tag.backgroundColor = RGBColor(204, 204, 204) ;
             }
             self.textView.autoresizingMask= NO;
             self.textViewHeight.constant = [self getTextHeight:KScreenWidth-40 text:self.model.descrip font:FontSize(17)]+10;
             self.textView.text = self.model.descrip ;
             if(arr2.count!=0){
